...
from package.fr.Localisation_Files_Invader import Localisation_Files_Invader
...
class Interface(Tk):
    def __init__(self):
        ...
        self.files_invader = self.create_panel_files_invader()
    ...
    def menu(self):
        ...
        menubar.add_command(label=Localisation_Files_Invader.PANEL_NAME.value, command=self.show_panel_invader)
        menubar.add_command(label=Localisation_About.PANEL_ABOUT.value, command=self.show_panel_about)
    ...
    def show_panel_about(self):
        ...
        self.files_invader.pack_forget()
    ...
    def show_panel_invader(self):
        self.panel_hidify.pack_forget()
        self.files_finder.pack_forget()
        self.files_invader.pack()
        self.panel_about.pack_forget()
    ...
    def create_panel_files_invader(self):
        p_files_invader = PanedWindow(self, orient=VERTICAL)
        p_input_file = PanedWindow(p_files_invader, orient=HORIZONTAL)
        self.input_invader_directory_path = Entry(p_input_file, textvariable=StringVar(), width=50)
        p_input_file.add(self.input_invader_directory_path)
        p_input_file.add(Button(p_files_invader, text=Localisation.TEXT_BROWSE.value, command=self.load_directory))
        p_files_invader.add(p_input_file)

        p_features = PanedWindow(p_files_invader, orient=HORIZONTAL)
        p_features.add(Button(p_features, text=Localisation_Files_Invader.BUTTON_POLLUTE.value, command=self.load_file))
        p_features.add(Button(p_features, text=Localisation_Files_Invader.BUTTON_UNPOLLUTE.value, command=self.load_file))
        p_files_invader.add(p_features)

        return p_files_invader


        p_hidify = PanedWindow(self, orient=VERTICAL)

        p_input_file = PanedWindow(p_hidify, orient=HORIZONTAL)
        p_input_file.add(Label(p_input_file, text=Localisation.PANEL_ARCHIVE_FILE_NAME.value))
        self.input_file_path = Entry(p_input_file, textvariable=StringVar(), width=50)
        p_input_file.add(self.input_file_path)
        p_input_file.add(Button(p_hidify, text=Localisation.TEXT_BROWSE.value, command=self.load_file))
    ...
    def launchArchiving(self):
        try:
            ...
            messagebox.showerror(Localisation.MESSAGE_ERROR.value,
                                 Localisation_Hidify.MESSAGE_EMPTY_NAME_FILE_OR_LOOP.value)
        except IOError:
                messagebox.showerror(Localisation.MESSAGE_ERROR.value, ExceptionMissingFile(input_file_path))
        except ExceptionMissingFile as e:
            messagebox.showerror(Localisation.MESSAGE_ERROR.value, e.value)
    ...
    def launch_unarchiving(self):
        try:
            ...
            messagebox.showinfo(Localisation.MESSAGE_INFORMATION.value, message)
        except ValueError:
            messagebox.showerror(Localisation.MESSAGE_WARNING.value,
                                 Localisation.MESSAGE_EMPTY_NAME.value)
        except IOError:
                messagebox.showerror(Localisation.MESSAGE_ERROR.value, ExceptionMissingFile(input_file_path))
        except ExceptionMissingFile as e:
            messagebox.showerror(Localisation.MESSAGE_ERROR.value, e.value)
        except tarfile.ReadError:
            messagebox.showerror(Localisation.MESSAGE_ERROR.value, ExceptionMissingTar(input_file_path))
    ...
    def launch_analyze(self):
        ...
        try:
            ...
            messagebox.showinfo(Localisation.MESSAGE_INFORMATION.value, Localisation_Files_Finder.MESSAGE_SUCCESS.value)
        except ValueError:
            print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + Localisation_Files_Finder.LOG_ERROR.value)
            messagebox.showerror(Localisation.MESSAGE_ERROR.value, Localisation_Files_Finder.LOG_ERROR.value)
            ...
            messagebox.showerror(Localisation.MESSAGE_ERROR.value, Localisation_Files_Finder(directory_path))
        except ExceptionPermissionDenied:
            print(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S") + " " +  str(Localisation_Files_Finder(directory_path)))
            messagebox.showerror(Localisation.MESSAGE_ERROR.value, ExceptionPermissionDenied(directory_path))